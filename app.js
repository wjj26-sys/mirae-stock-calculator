...
function calcCard(card) {
  ...
  const profit = (nowPrice - entryPrice) * qty;    // 평가손익
  const evalAmt = buyAmt + profit;                 // 평가금액 = 보유수량 × 현재가
  const rate = buyAmt !== 0 ? (profit / buyAmt) * 100 : 0; // 수익률
...
function calcSummary() {
  ...
  const buy = cards.reduce((sum, card) => sum + card.buyAmt, 0);       // 상단 매입금액
  const evalAmt = cards.reduce((sum, card) => sum + card.evalAmt, 0);  // 상단 평가금액
  const profit = cards.reduce((sum, card) => sum + card.profit, 0);    // 상단 평가손익
  const rate = buy !== 0 ? (profit / buy) * 100 : 0;                   // 상단 수익률
...
function renderCards() {
  ...
  cards.forEach((card, index) => {
    ...
    // 매입금액 = 보유수량 × 진입가
    renderDigitString(document.getElementById(`sell-${index}`), formatInt(card.buyAmt), "black");
